...
exports.viewAttendance = async (req, res) => {
  ...
  exports.attendance_page = async (req, res) => {
  ...
  exports.attendanceInfo = async (req, res) => {
    ...
    // Pagination setup
    let perPage = 12; // Number of records per page
    let page = req.query.page || 1; // Page number from query string
  
    try {
      // Query to retrieve attendance records
      const attendanceRecords = await Attendance.aggregate([{ $sort: { date: -1 } }]) // Adjust sorting field as needed
        .skip(perPage * (page - 1)) // Correct calculation for pagination
      ...
      // Render the view with the attendance data
      res.render("attendance_info", {
        locals,
        attendanceRecords, // Send attendance records to the view
