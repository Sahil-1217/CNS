...
exports.data = async (req, res) => {
  ...
  try {
    const material = await Sites_info.aggregate([{ $sort: { SiteNumber1: 1 } }])
    ...
    res.render("index1", {
      locals,
      sites: material, // Changed variable name to sitesInfoData
      current: page,
...
//const Attendance = require('../models/Attendance'); // Adjust the path as needed
    ...
    exports.info1 = async (req, res) => {
      ...
      try {
        const material = await Material.aggregate([{ $sort: { createdAt: -1 } }])
          .skip(perPage * page - perPage)
          .limit(perPage)
          .exec();
  ...
  //const Attendance = require('../models/attendance'); // Make sure to require your Attendance model
